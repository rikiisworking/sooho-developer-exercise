...
describe("RewardToken", () => {
  ...
  it("deposit() should mint RewardToken to user", async () => {
    ...
    await rewardToken.connect(user0).balanceOf(user0).then((result: bigint) => {
  ...
  it("withdraw() should transfer Ether to user", async () => {
    ...
    await rewardToken.connect(user0).balanceOf(user0).then((currentTokenBalance: bigint) => {
  ...
  it("mint() should return error if totalSupply exceeds maxSupply", async () => {
    ...
    await rewardToken.connect(owner).setMinter(minter);
    await expect(rewardToken.connect(minter).mint(user0, maxSupply + mintAmount)).to.be.revertedWith("max supply limit violated")
  })
  ...
  it("updateSwapRatio() can be called only by owner", async () => {
    await expect(rewardToken.connect(user0).updateSwapRatio(ethers.parseUnits("100", 12))).to.be.revertedWith("Ownable: caller is not the owner")
  })
  ...
  it("updateSwapRatio() should return error if input ratio is bigger than realRatio", async () => {
    await rewardToken.connect(user0).deposit({value: depositAmount});
    const realRatioBefore = await rewardToken.realRatio()
    await expect(rewardToken.connect(owner).updateSwapRatio(realRatioBefore * BigInt(2))).to.be.revertedWith("invalid input ratio");
  })

...
describe("Bank", () => {
  ...
  it("deposit() should revert if sidecar has been activated", async () => {
    ...
    await expect(bank.connect(user0).deposit({value: depositAmount})).to.be.revertedWith("sidecar has been activated");
  })
  ...
  it("withdraw() should revert if withdraw amount is greater than balance", async () => {
    await bank.connect(user0).deposit({value: depositAmount});
    await expect(bank.connect(user0).withdraw(ethers.parseEther("100"))).to.be.revertedWith("withdraw amount exceeded balance");
  })
  ...
  it("stake() should work as expected", async () => {
    await bank.connect(user0).deposit({value: depositAmount});
    const tx = await bank.connect(user0).stake(stakeAmount);
    const result = await tx.wait();
    if (result) {
  ...
  it("stake() should revert if stake amount is greater than deposit amount", async () => {
    await bank.connect(user0).deposit({value: depositAmount});
    await expect(bank.connect(user0).stake(depositAmount * BigInt(2))).to.be.revertedWith("stake amount exceeded deposit amount");
  })
  ...
  it("unstake() should revert if reward claimed in 24h", async () => {
    await bank.connect(user0).deposit({value: depositAmount});
    await bank.connect(user0).stake(stakeAmount);
    await expect(bank.connect(user0).unstake(stakeAmount / BigInt(2))).to.be.revertedWith("cannot unstake for 24h after reward claimed");
  })
  ...
  it("unstake() should revert if unstake amount is greater than stake amount", async () => {
    ...
    await bank.connect(user0).stake(stakeAmount);
    await time.increase(duration.days(1));
    await expect(bank.connect(user0).unstake(stakeAmount * BigInt(2))).to.be.revertedWith("unstake amount exceeded stake amount");
  })
  ...
  it("claimReward() should work as expected", async () => {
    ...
    const expectedAmount = (stakeAmount * BigInt(2) / BigInt(duration.days(365))) * (BigInt(duration.days(1))) / BigInt(10**12);
  ...
  it("withdrawPotMoney() can be called only by owner", async () => {
    await bank.connect(owner).depositPotMoney({value: ethers.parseEther("10")});
    await expect(bank.connect(user0).withdrawPotMoney(ethers.parseEther("5"))).to.be.revertedWith("Ownable: caller is not the owner");
  })

  it("withdrawPotMoney() should revert if withdraw amount is greater than potMoney balance", async () => {
    await bank.connect(owner).depositPotMoney({value: ethers.parseEther("10")});
    await expect(bank.connect(owner).withdrawPotMoney(ethers.parseEther("15"))).to.be.revertedWith("withdraw amount exceeded potMoney balance");
  })
  ...
  it("invokeSideCar() can be called only by owner", async () => {
    await expect(bank.connect(user0).invokeSidecar(duration.hours(1))).to.be.revertedWith("Ownable: caller is not the owner");
  })
